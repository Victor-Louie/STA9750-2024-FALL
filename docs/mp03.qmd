...
```{r message=FALSE}
list(gained_seat_plot, lost_seat_plot)
```
...
ggplot(top_15_votes_data, aes(x = reorder(state, -votes), y = votes, fill = vote_type)) +
...
ggplot(ecv_map, aes(geometry = geometry, label = electoral_votes)) + 
